...
- Hazards in play: HZ-001 (RF exposure) scales with band; none new
...
|---|---|---|
| REQ-SYS-NNN (frequency coverage 144.000 to 148.000 MHz; allocated by the L1 author) | new, traces to SI-002 and SI-024 with this ADR in `source_ids` as supporting source | See ADR-016 for the band-edge consequences |
| every L1 and L2 requirement | tagging rule: `revA` for band-specific, `70cm-ready` for band-independent | Reviewer checks the tag at requirements validation |
