...
Environment: handheld and table-top use outdoors; operating ambient -10 C to +45 C (REQ-SYS-114, TBR, close by PDR; `docs/plan/tpm.json` TPM-006 carries the same span), bounded by the -10 to +60 C display window, the encoder rating and the TCXO range (`docs/research/display-and-ui-parts.md` F2, F12); charging 0 to 45 C; light rain when upright with plugs inserted, quantified as 10 min of IEC 60529 IPX2 dripping water (REQ-SYS-117, TBR, close by PDR), with the USB opening, jacks and encoder bushings unsealed (not water resistant). 40 C is the ambient of the PA thermal analysis case (section 7.8), not the operating limit.
...
On transmit, the keyer (in firmware, timed from a hardware timer independent of the user interface) sequences the T/R relay to the transmit position, waits for contact settling, then raises the carrier through a 5 ms raised-cosine envelope produced by the gate-bias ALC loop of the PA; the loop also holds the selected power step (0.5, 1, 2 or 5 W) constant across the 6.4 to 8.4 V pack range. The harmonic low-pass filter sits at the antenna port after the T/R node. After the last element and an adjustable hang time the radio returns to receive (semi break-in, SI-036). Independent of firmware, a hardware monostable removes PA enable if a key-down persists longer than about 10 s, the PA is inhibited by hardware whenever USB VBUS is present (sensed from the USB input B16, not from the charger state, so the inhibit holds while charging is paused in Receive; REQ-SYS-092), and every reset leaves the transmit lines pulled low.

Power comes from two 18650 cells in series behind a two-cell protector; the PA runs from the protected pack, a synchronous buck makes 5 V for the Pico 2 and audio, and a low-noise LDO makes 3.3 V for the receiver and synthesizer. The charger takes 5 V from the Pico 2 micro-USB at 500 mA by default; with the power switch on and USB present the radio receives and charging is paused (REQ-SYS-093); transmit is inhibited whenever USB power is present. A mechanical switch controls power.
...
The operating behaviour is the one of `docs/conops/conops.md` section 3.4, which REQ-SYS-002 makes binding: nine modes (Table 3.4-1), four flags (Table 3.4-3), three inhibits and the cause classes of Table 3.4-4, and the Arm condition. This section adds no state of its own; it maps the concept's means onto those modes and names the implementation sub-states the design proposes inside them. Sub-state names are written in the CamelCase enum spelling that 02 §4.2 WR-07 prescribes for architecture state names; they are `[Proposed, confirmed by the architecture at PDR]` and are not modes.

| ConOps mode, flag or inhibit | Concept means | Implementation sub-states (proposed) |
|---|---|---|
| Off | Mechanical switch open; charger and protector live at about 25 uA; transmit lines held low by the pull-downs (section 7.5) | none |
| Charging | USB present with the switch off; controller and display on USB power, radio functions unpowered; the B07 VBUS inhibit holds the PA path off | none |
| Self-test | Clocks and TICKS started, 2 s watchdog armed, safe pad states, configuration record recovery, the checks of Table 3.4-1; the KEY interlock (Table 3.4-4 row 1) starts here | `Boot` (clock, watchdog and pad set-up before the checks run) |
| Receive | Nominal listening; Arm evaluated from the flags and inhibits below | none |
| Transmit-keyed | Relay to transmit, first-element lead-in, shaped elements, hang | `TxPending` (relay operate and settling during the lead-in), `TxElement` (elements with envelope), `Hang` (semi break-in hang, default 8 dits, REQ-SYS-044); all three are sub-states of Transmit-keyed |
| Tune | 0.5 W carrier ending at most 5.5 s after it starts, nominal 5 s +/-0.5 s, before the 7.5 s floor of the hardware cutoff (REQ-SYS-020, TBR, close by PDR) `[Proposed, owner decision pending at SRR]` | none |
| Bench-test | The test functions of ConOps section 3.5.2; every timeout, the cutoff, every flag and inhibit stay active | none |
| Firmware-update | Bootrom UF2 load; the transmitter cannot be enabled (REQ-SYS-119) | none |
| Fault-safe (latched) | Entered only by the Latched class (Table 3.4-4 rows 10 to 15; T06, T09, T17): the full safe state first (PA enable low, relay unpowered to receive, audio muted, charging disabled, REQ-SYS-130), then the cause shown | none |
| Flags GUEST, PRACTICE, USB, LOWBATT | Each removes Arm (Table 3.4-3); USB also acts through the B07 hardware VBUS inhibit (REQ-SYS-092) | flag bits, not states |
| Inhibits KEY, HOT, GUARD | Self-clearing: transmit part only (amplifier off, key up, antenna to receive, REQ-SYS-004); the unit stays in Receive; the stuck-key sidetone of rows 2 and 3 keeps sounding | inhibit bits, not states |

A controller reset (watchdog, brown-out, static discharge) returns through Self-test (T18, Table 3.4-4 row 9) and is not a latched state; only a second watchdog reset without a switch-off latches Fault-safe (row 10).
...
flowchart LR
  ...
  subgraph TX["TX: transmitter and frequency generation"]
    ...
    CUT["Hardware PA-enable cutoff, T_max 10 s, TX inhibit while VBUS present (B07)"]
  end
...
```
 SMA (B01) --- LPF (B02) --- T/R relay (B03) --RX-- BPF+LNA (B08) -- Mixer (B09) --+-- A: xtal filter, IF, AGC, detector (B10) -- audio --> Pico 2 (B12)
   ...
   |                              Hardware cutoff (B07): TX_KEY AND monostable Q AND NOT VBUS (from B16)             |  UART0 trace / USB bootrom (ICD-SW-HOST)
   |                                                                                                                 |
...
Rendering note (charter §11 rule 3): the Mermaid block above is the text source of `docs/reviews/SRR/figures/concept-block-diagram.png`. The render is drawn by `docs/reviews/SRR/figures/concept-block-diagram.py` (matplotlib, headless), which reads this block at run time (blocks, module groups, edges, labels and arrow kinds) and holds only the layout; it exits 1 when this block and its layout disagree, so a change to this block is followed by a layout update, a re-render and a new inspection. In the render the two 3.3 V analog edges from B20 are drawn as rail connectors, and B01 sits outside the module groups as it does here. Rendered and inspected 2026-09-25, and again after the B07 inhibit source moved from the charger to VBUS presence (B16) in response to INSP-002 finding-3. The ASCII sketch is the fallback where the render is unavailable.
...
| F3 Key and sequence | F3.1 sense two contacts with 2 ms close and 5 ms open debounce (TBR, close by PDR); F3.2 generate elements in Straight, Iambic A (default), Iambic B, Ultimatic and Bug modes, 5 to 50 WPM, default 15; F3.3 hold element timing to +/-1 % or +/-0.5 ms, paddle-to-element latency at most 3 ms; F3.4 sequence relay, PA bias and envelope (first-element lead-in 8 to 12 ms, at most 12 ms by REQ-SYS-161, TBR, close by PDR; envelope fall complete before relay release); F3.5 sidetone 600 Hz default, 300 to 1000 Hz, locked to the receiver offset, hang 8 dits default; F3.6 enforce the stuck-key set: firmware straight-key timeout 5 s (REQ-SYS-053, TBR), paddle watchdog 128 consecutive identical elements or 10 s of them, elapsed time from the first of those elements (REQ-SYS-054, TBR, close by PDR; 30 s is the alternative of SRR package decision 37, D-KN3, pending the owner), key-closed-at-power-on interlock (each input the mode uses open at least 500 ms) | SW (F3.1 to F3.6), CTL (input network, sample path), TX (relay and PA control lines) | SI-018, SI-033, SI-036; `docs/research/keyer-verification-and-key-input-network.md` F4, F6, F7, F11, F13, F14 |
...
| F9 Protect people and comply | F9.1 RF exposure controls: power steps, 1 W first-boot default, 0.5 W tune carrier ending at most 5.5 s after it starts (REQ-SYS-020, TBR, close by PDR), posture line, manual section, engraved legend; F9.2 acoustic ceiling in hardware; F9.3 PA thermal limits (junction at most 110 C by analysis at 5 W, 40 C ambient, continuous key-down); F9.4 band-edge guard and only A1A; F9.5 identification reminder at 9 min 00 s +/-5 s (REQ-SYS-068, TBR), with no automatic identification in rev A (REQ-SYS-007; a CW ID capped at 20 WPM by 47 CFR 97.119(b)(1) is a deferred capability, section 12); F9.6 licensed-operator model and guest lock | SW (F9.1 logic, F9.4, F9.5, F9.6), CTL (F9.2), TX and ME (F9.3), ME (legend) | 47 CFR 97.119(b)(1), 97.313(a), 97.13(c); `docs/research/rf-exposure-evaluation.md` F2, F3, F7; `docs/research/pcbway-export-and-vendor-questions.md` F17 |
...
| CTL | B12 to B15 | Pico 2 module and its clocks, key input network and ESD clamps, display and ZIF, encoders, buttons, jacks with detect, audio DAC path and amplifier, the TX_KEY output and its pull-down at the controller pin, USB connector exposure; the cutoff block B07 is allocated to TX and CTL reaches it through `ICD-TX-CTL` |
...
|---|---|
| Unintended or prolonged transmission | Interlock at boot and after any reset (both key inputs open at least 500 ms); firmware straight-key timeout 5 s (REQ-SYS-053, TBR) and paddle watchdog 128 identical elements or 10 s of them (REQ-SYS-054, TBR; 30 s alternative pending D-KN3); hardware monostable cutoff T_max 10 s acting on the envelope modulator control; 2 s CPU watchdog; reset pad states plus 4.7 kOhm pull-downs on TX_KEY and PA_EN; relay unpowered state is receive; transmit inhibited by hardware whenever USB VBUS is present (B07, REQ-SYS-092); band-edge guard and receive-only guest lock in firmware (`docs/research/keyer-verification-and-key-input-network.md` F13) |
| RF exposure | Power steps with 1 W default and deliberate 5 W selection; 0.5 W tune carrier ending at most 5.5 s after it starts (REQ-SYS-020, TBR, close by PDR); posture line on the display; manual and engraved legend; mobile-leaning posture in the ConOps with handheld-at-face as off-nominal evaluated by SAR analogy `[Proposed, owner decision pending at SRR]` |
...
| `ICD-TX-ANT` | External | SMA jack female body, 50 ohm, 1/4-36 thread; at least 4.0 W (TBR) into SWR 2:1 (REQ-SYS-152), survives 60 s into 10:1, open and short (REQ-SYS-013); bending moment 4.0 N.m and 500 matings (REQ-SYS-106) carried by the enclosure boss; counterpoise lug; ME cites it for the boss | before SRR |
| `ICD-CTL-USB` | External | Pico 2 micro-USB: 5 V VBUS to the charger, at most 500 mA (TBR, REQ-SYS-090); a higher input current up to 1.5 A applies only if the TS-005 USB input policy study adopts it by CR; bootrom UF2 load; enclosure opening 12.0 x 10.0 mm; PWR cites it for charge power | before SRR |
...
| `ICD-TX-PWR` | Internal | PA rail from the protected pack, relay coil supply, VBUS-present inhibit line from B16 to B07, 3.3 V for the synthesizer | PDR |
...
2. TS-NNN-synthesizer-reference scores each synthesizer on 70 cm output capability: LMX2571 covers 10 to 1344 MHz directly; Si5351A tops out at 200 MHz and would need a multiplier or a different LO plan (`docs/research/2m-cw-transceiver-reference-designs.md` F16, F20).
...
| Keyer reference model and golden vectors (`tools/keyer_ref.py`, `docs/test_cases/sw-keyer/keyer-golden-vectors.json`) | Compare the Rust keyer against an independent reference, not two hand-written copies | Proposed (A-KN1); vectors defined (F7) | PDR (test cases), CDR (implemented) |
...
2. Power steps 0.5, 1, 2 and 5 W within +/-1 dB; 1 W at first boot and after reset; deliberate action to select 5 W; tune carrier 0.5 W ending at most 5.5 s after it starts, nominal 5 s +/-0.5 s, below the 7.5 s floor of the hardware cutoff (REQ-SYS-020, TBR, close by PDR; RFX-D3 with D-KN2).
3. Stuck-key set: firmware straight-key timeout 5 s; paddle watchdog 128 consecutive identical elements or 10 s of them in elapsed time (REQ-SYS-054, TBR), or 30 s with MOE-012 amended (package decision 37, D-KN3; the owner chooses); hardware cutoff T_max 10 s (7.5 to 13 s); interlock each input the mode uses open at least 500 ms.
