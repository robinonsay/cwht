#!/bin/bash
# venv Python + jsonschema sanity check runner (TV-001 section 3 procedure of 2026-09-26, unchanged, re-run for SRR package item R5 on a tree given as $1, default the repository; transcript kept as evidence).
PY=/Users/robinonsay/rust/cwht/.venv/bin/python
export TREE
TREE=${1:-/Users/robinonsay/rust/cwht}
FX=$TREE/tools/tests/fixtures/schema
W=$(mktemp -d "${TMPDIR:-/tmp}/kat-jsonschema.XXXXXX")
echo "# venv Python + jsonschema sanity check, $(date '+%Y-%m-%d %H:%M:%S %Z'), host $(uname -srm)"
echo "# repository HEAD $(git -C /Users/robinonsay/rust/cwht rev-parse HEAD); tree tested $TREE"
(cd $FX && shasum -a 256 *.json)
echo "\$ cmp tools/tests/fixtures/schema/requirements.schema.json docs/requirements/schema.json"; cmp $FX/requirements.schema.json $TREE/docs/requirements/schema.json && echo "identical (informative: the fixture copy still equals the repository schema)"
...
def kw(n,out):
    if isinstance(n,dict):
        ...
        for x in n: kw(x,out)
import os; root=os.environ["TREE"]+"/"; files=sorted(glob.glob(root+"docs/**/*schema*.json",recursive=True))
repo=set()
for f in files:
