...
# as one KAT line. Adaptation 2 (after run 1 failed): FreeCAD 1.1.3 returns the Part::Cut result as a
# Compound holding one Solid, which the listing's ShapeType assertion rejects; a Compound with exactly
# one solid is replaced by that solid before the assertions (line marked ADAPTATION).
# It is not tools/scad2step.py (not yet written; CM plan section 8.2 step 3).
...
print("KAT root ShapeType before adaptation:", shape.ShapeType, "solids:", len(shape.Solids))
if shape.ShapeType == "Compound" and len(shape.Solids) == 1: shape = shape.Solids[0]   # ADAPTATION
