...
| NASA-HDBK-2203 (SWEHB Ver D) | Guidance, rationale, small-project tailoring and software-assurance tabs per SWE | `docs/references/md/swehb/` (scraped 2026-09-25: 267 files, 130 of them SWE pages; per-SWE completeness check tracked by RSK-009) |
...
| E / F — Operations, closeout | On-air use by owner and friends, anomaly reports, lessons learned, rev B formulation | Periodic status; DR and DRR not held; SE-55 and SE-56 tailored by deviation (compliance matrix) | — |

Entrance and success criteria for SRR, PDR, CDR, TRR and SAR are tailored from NPR 7123.1D App. G Tables G-4, G-6, G-7, G-10 and G-11, with the minimum products of NPR 7123.1D §5.2.2.2 mapped to project artifacts as customization of combined reviews: SRR carries SE-38/39/66 plus the MCR products SE-35/36/37; PDR carries SE-45/67/68 plus the SDR products SE-40/41/42/43; CDR carries SE-46; TRR carries the SIR products SE-47/48; SAR carries the ORR and FRR products SE-69, SE-51, SE-52, SE-53, SE-54. SE-44, SE-51 and SE-52 are dispositioned in the compliance matrix. A **delta TRR** (`TRR-Dn`) precedes every new run-for-record series, including the first on-air series, which requires all Part 97 emission cases passed. Runs on a bare Pico 2 development board before hardware arrives are recorded as Bench evidence with `credit: false` (setup-line convention of `docs/process/04-verification-and-validation.md` §4 until a schema CR adds a field); TRR gates the delivered unit. The tailored criteria live in `docs/process/01-lifecycle-and-reviews.md`.
...
4. **Completion** (NPR 7123.1D §5.2.3.1): all RIDs/RFAs dispositioned or on an agreed closure plan; review minutes and decision memo written (`docs/reviews/<REVIEW>/decision-memo.md`); baseline tagged in git (`baseline/srr`, `baseline/pdr`, …). Delta reviews use the token `TRR-Dn` (folder `docs/reviews/TRR-Dn/`, no baseline tag). Between reviews, an approved Change Request's disposition block and an ADR's decision section are the decision memos for that change or decision, including risk acceptances and approvals of Red-risk mitigation plans (`docs/process/06-risk-and-decision-analysis.md` §8); chat approvals are transcribed into them.
...
| Software Requirements Specification | `docs/requirements/sw/sw-<sub>/requirements.json` (e.g. `sw-keyer`, module `SW-KEYER`) |
...
| Build-to specifications and analyses | `docs/design/build-to-specification.md` (includes design-for-debug and producibility rules), `docs/design/analysis/` |
| RF Exposure Evaluation (controlled regulatory document, 47 CFR 97.13 and 1.1307; HZ-001) | `docs/design/analysis/rf-exposure-evaluation.md` |
...
| Deviations log | `docs/cm/deviations.md` |
| Unsafe-code audit list (generated) | `firmware/unsafe-audit.md` |
...
| Per-review working files | `docs/reviews/<REVIEW>/{minutes.md, checklists/<product-slug>.md, figures/, traceability-report.md, traceability.json, baseline-record.md}` (the two traceability files are written by the command of `docs/process/01-lifecycle-and-reviews.md` §3.1 item 2); each filled checklist is the single peer-review record for its product and carries `id: INSP-NNN` in its front matter (there is no separate `peer-reviews/` folder) |
...
| Hardware release packages (fab, assembly, CNC, PCBWay and DigiKey order confirmations, owner hand-assembly list) | `hardware/releases/HW-MB-rev<X>-<n>/`, with the SMT versus hand-solder split in `cwht-MB-rev<X>-hand-assembly.csv` (SI-031) |
...
`NGO-NNN` need/goal/objective · `CON-NNN` stakeholder constraint · `SI-NNN` stakeholder input · `MOE-NNN` · `MOP-NNN` · `TPM-NNN` · `OPS-NNN` ConOps scenario · `REQ-<MOD>-NNN` requirement (modules: SYS, RX, TX, PWR, CTL, ME, SW for firmware-wide software requirements, SW-<sub>) · `TC-<MOD>-NNN` test/verification case (requirement modules plus the test-only modules VAL, ATP, SW-COV, SW-REG, SW-TOOL of `docs/process/04-verification-and-validation.md` §1) · `ICD-<A>-<B>` interface · `ADR-NNN` decision record · `TS-NNN` trade study · `RSK-NNN` risk · `HZ-NNN` hazard · `NCR-NNN` nonconformance · `CR-NNN` change request · `RFA-<REV>-NNN` / `RID-<REV>-NNN` where `<REV>` is SRR, PDR, CDR, TRR, TRR-Dn or SAR · `TV-NNN` tool validation record · `ACC-<TOOL>-NNN` accreditation scope statement, held inside its TV-NNN record (e.g. ACC-EMU-001) · `INSP-NNN` peer review record · `MSR-NN` software measurement · `CS-NN` coding standard rule · `WP-SW-NN` rustos driver work package · `SW-NN-<module>` software sprint · `OQ-<AREA>-NNN` open question · `CWHT-A-NNN` unit serial · `<TC-ID>-rN` verification report. Requirement modules are SYS, RX, TX, PWR, CTL, ME, SW (firmware-wide, `docs/requirements/sw/requirements.json`) and SW-<SUB> laid out as `docs/requirements/sw/sw-<sub>/requirements.json` (module id is the upper-cased directory name, `sw-keyer` gives `SW-KEYER`; test cases in `docs/test_cases/sw-<sub>/`); VER is reserved and unused. IDs are never reused; a retired item keeps its ID with status *Retired* where its schema has that status, otherwise status *Closed* with tag `retired`, plus a rationale; test cases, whose schema has neither, use status *Blocked* with a setup line beginning `Retired by CR-NNN` (`docs/process/02-requirements-and-traceability.md`).
...
| CMMI rating (SWE-032), IV&V (SWE-141, 131, 178, 179), Center reporting (SWE-174), architecture review board (SWE-143) | Not applicable or tailored as recorded in the RMM | Presuppose NASA institutions. Intent preserved by independent agent review and a repo-local measurements file. Chapter-2 Center Director requirements (SWE-006, 091, 092, 142, 144, 214 to 217) have no project row in App. C and are institutional, outside the RMM. |
| Software assurance and safety standards (SWE-022, SWE-023, SWE-017 training) | Tailored | NASA-STD-8739.8 is not in the corpus; the safety-critical determination uses NPR 7150.2D's own criteria and SWEHB software-assurance tabs; SWE-017: there are no project personnel to train in the NPR sense (agents are instantiated per task and the owner is the only human); the intent is met by brief-time reading lists (`docs/process/08-agent-briefing.md` §2). |
...
| Non-custom software testing (SWE-211) | Tailored, proposed for owner decision at SRR | Structural coverage of Rust `core` (the OSS runtime shipped with the locked rustc, neither instrumented nor modified) is relieved; its used features are tested at feature level (`docs/process/04-verification-and-validation.md` §4). rustos `api` and `pico2` are tested to the custom-code level in full. |
...
| Spectrum manager concurrence, JCL, ILSP, human rating, project protection plan, launch-site items (NPR 7123.1D App. G guidance rows) | Customized (SE HB §3.11.4.3) | App. G rows are guidance, not SE-NN requirements; their omission is recorded in the review plan and SEMP. Owner is the FCC licensee; Part 97 compliance requirements replace spectrum-manager items. Compliance-matrix rows are needed for SE-44 (not applicable: program-only by the NPR's own App. H rationale), SE-51 and SE-52 (tailored: ORR combined with SAR; scope relief to the operations-handbook end-of-life and 18650 cell-disposal section relative to App. G Tables G-17 and G-18), SE-55 and SE-56 (tailored by deviation: no DR or DRR is held) and SE-62 (mass margin, fully compliant: a handheld carries a mass TPM, provisional allocation 350 g). |
...
| Reuse cataloging and Government rights (SWE-147, 148) | Not applicable | Open personal project; license recorded in the repo. |
