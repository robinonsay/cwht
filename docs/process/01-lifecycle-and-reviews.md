...
| 28 | Regulatory corpus present so that Part 97 clauses are cited from the repository | SEMP section 2.0 (due before PDR) | `docs/references/md/regulatory/`: 47 CFR Parts 1, 2, 15 and 97 verbatim at eCFR issue 2026-09-23, fetched with the eCFR versioner API command recorded in its `README.md`, plus the FCC extracts; the `README.md` indexes every file and gives the currency check | Corpus file list from the `README.md` index, or the lien with its PDR due event | Soft (Hard at PDR, section 5.3 row 12) |
...
| G-3 5.8 "Technology Development Plan" | No baseline technology is below TRL 6; the one sub-TRL-6 technique, the candidate-B PIO I2S receive path (technology TRL 5, a non-baseline candidate of TS-001), is dispositioned in technology assessment section 7 (row 20); a further gap becomes a risk and a research report. For the same reason there is no plan to update at SRR (G-4 6.16, updated Technology Development Plan, row 20) |
