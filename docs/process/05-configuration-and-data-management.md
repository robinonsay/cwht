...
| 13 | Architecture decision records | `docs/decisions/adr/` | ADR-NNN | Record | n/a | Superseded, never edited; the status field is the only editable line (Accepted, Superseded by ADR-MMM). One exception, before the `baseline/srr` tag only: the one-time pre-baseline correction of the Accepted ADRs found by INSP-011 (`docs/decisions/adr/reconciliation-srr.md` section 7 ruling R-1 option (A)) may fold the reconciled decision class, assumptions, hazard, section 4.1, errata and reviewer lines into the ADR files, never section 2 (Decision), under that INSP record with the owner's approval in the SRR decision memo; every later edit follows the rule above. |
...
**Table 4-2. Baseline admission evidence (functional baseline, SRR).** Each CR-class row of the Functional line above enters `baseline/srr` only with the evidence below. An `INSP-NNN` record is required where a checklist for the product type exists (`docs/process/08-agent-briefing.md` §3.5); where none exists, or the CI is the owner's own direction, the evidence named here admits it and no review is held without a checklist (08 §3.5). Records marked "not yet in H1" are not yet in the record plan of `docs/reviews/SRR/package.md` §2 item H1 and are added to it by Claude (cross item of this revision). The allocated, product and as-built baselines get their own Table 4-2 rows by a Class II CR against this plan before the PDR readiness declaration.

| Row | CI | Admission evidence | Record or check |
|---|---|---|---|
| 1 | Process charter | Owner approval of the charter in `docs/reviews/SRR/decision-memo.md`. No `INSP-NNN`: the charter is the owner's direction against which every other record is reviewed (charter §2, §5). | Decision memo item naming the charter commit |
| 2 | 01, 02, 04 to 08 process documents and the SEMP | One `INSP-NNN` per document, checklist `peer-review-checklist-requirements.md` section G. | 05: `INSP-006`; SEMP and 07: in H1; 01, 02, 04, 06, 08: not yet in H1 |
| 2 | `docs/plan/schedule.md`, `docs/plan/cost-estimate.md` | One `INSP-NNN` each, checklist section G (tailored SWE-016 and SWE-015 plans, charter §12). | Not yet in H1 |
| 2 | `docs/process/README.md` | Index only, no technical content: the §7.4 interim check confirms that it names every file of rows 1 to 3 and nothing else. | §7.4 check result in the baseline record §2a |
| 3 | 03 classification record and `rmm.json` | The classification concurrence `INSP-NNN` with a software assurance reviewer (SWE-176), plus `tools/render_rmm.py --check` exit 0 at the baseline commit. | In H1; tool run in the baseline record |
| 3 | Compliance matrix `se-compliance-matrix.json` | `INSP-NNN`, checklist section G (item CK-REQ-G4 mirrors tailoring), plus `tools/render_compliance.py --check` exit 0. | Not yet in H1 |
| 3 | `rmm.md`, `se-compliance-matrix.md` | Rendered: `--check` exit 0 of the two renderers shows the committed file equals the render. | Tool runs in the baseline record |
| 5, 6, 7, 51, 52 | Expectations, ConOps, L1 requirements, technology assessment, concept | One `INSP-NNN` per product as H1 lists them (`requirements-sys.md` with the V1 to V6 validation of every L1 requirement). | In H1 |
| 9 | Schemas | No checklist exists for schemas. `tools/validate_docs.py` exit 0 and `python -m unittest discover -s tools/tests` all passing at the baseline commit, with the Accredited TV records of `tools/validate_docs.py` and the venv Python with `jsonschema` (§13 SRR row). | Tool runs and TV ids in the baseline record |
| 17 | Test cases citing L1 requirements | One `INSP-NNN` for `docs/test_cases/sys/test_cases.json`, checklist `peer-review-checklist-test.md`, plus `tools/traceability.py` with zero violations for the `TC-SYS-*` ids. | Not yet in H1 |
| 27 | `tools/toolchain.lock.md`, `tools/requirements.txt` | No checklist exists for the lock. The Accredited SRR TV records of §13 (each cites the lock line it validates), the lock §1.1 sanity-check results, and the AL-4 check that `tools/requirements.txt` equals `.venv/bin/pip freeze`. | TV ids and the AL-4 comparison in the baseline record |
| 53 | Templates and peer-review checklists | No checklist exists for templates. `tools/validate_docs.py` exit 0 (the example files validate against their schemas) and the unit tests passing (`PeerReviewRecordTests` read the checklist front matter); each peer-review checklist is admitted at the revision cited by the filed SRR `INSP-NNN` records that used it, and a defect found in use is a RID against row 53. | Tool runs and the checklist revisions in the baseline record |
...
1. Confirm `main` is clean, `tools/traceability.py` passes, `git fsck --full` exits 0, every tracked file matches a Table 4-1 row (§4.2), and every CI listed for the baseline holds the admission evidence of its row in Table 4-2 (below) with no open Major RID against it. A CI whose Table 4-2 evidence is an `INSP-NNN` record is at L1 (filled checklist with `id: INSP-NNN`, no open Major finding); a CI admitted by other evidence is admitted by that evidence alone, and the baseline record §2a cites it. A CI that lacks its evidence is either left out of the baseline by the owner in the decision memo (it stays at L0 or L1 and is listed as a lien) or admitted under a numbered departure entered in `docs/cm/deviations.md` before the tag; nothing is tagged on an unlogged departure.
...
Cycle-time targets: Class II dispositioned within one working session of submission; Class I within the next review or 14 days, whichever is earlier. These are targets, not commitments (charter §12 schedules row); a CR over its target is flagged and handled by the thresholds and responses of Table 6-1 (§6, CSA item 10).
...
| 10 | Metrics | The four CM metrics of Table 6-1, each with its value, threshold and state at the generation date. | `docs/plan/measurements.json`; derived per Table 6-1 |
...
**Table 6-1. CM metrics (CSA item 10).** Each metric is computed at every CSA issue; the value history is the committed versions of `docs/process/configuration-status.md` (row 35) and the copy in each review package. A breach is reported in the next review package and handled as the Response column states; the owner adopts RIDs and RFAs per 01 section 10.1.

| Metric | Computation and source | Threshold | Response | Stored in |
|---|---|---|---|---|
| Requirements volatility | §5.4: latest `MSR-02` | 10 % yellow, 20 % red (07 §11.2) | Red: RFA at the next review and a risk per 06 (§5.4) | `docs/plan/measurements.json` (`MSR-02`), copied into CSA item 10 |
| Open CRs and their age | Count of CRs in Draft, Submitted, Assessed or Deferred from the CR front matter (CSA item 4); age = generation date minus `date_opened` in days; for a Submitted or Assessed CR also the days since submission | Zero CRs past their §5.2 cycle-time target; zero Deferred CRs past their re-look-by review | Each overdue CR is listed by id in the next package; the owner dispositions or re-defers it at that review, or an RFA is raised for it | CSA items 4 and 10 |
| CR cycle time | Per class, days from Submitted to Dispositioned for every CR dispositioned since the previous baseline (CR history section) | Class II: one working session; Class I: the next review or 14 days, whichever is earlier (§5.2) | Every CR over target is listed with its cycle time; two or more over target in one review interval raise an RFA on CCB throughput at the next review | CSA item 10 |
| Commits lacking mandatory trailers | Count of commits in `<prev-baseline>..HEAD` that touch a CI without a `Refs:` trailer, or touch a class-CR CI after its CR-from event without a `CR:` or `Editorial:` trailer (§4.5 Checks) | 0 (SEMP §5 CM row: CIs changed after their baseline without a CR or `Editorial:` trailer: 0) | Each such commit is a RID at the next review; one touching a class-CR CI without a CR is also a deviation in `docs/cm/deviations.md` (CSA item 11) | CSA items 5, 10, 11 |
...
Build flavours. A credited Bench run on an instrumented build (sample-marker and report-marker test pads) or a fault-injection build (injection commands with an injection marker) is a run on a configuration-identified build (SWE-187): each flavour of a release is built by the same script at S with its Cargo feature named in the VDD, written to `firmware/releases/vX.Y.Z/flavours/<flavour>/` with its own ELF and UF2, listed in `SHA256SUMS`, and carried in VDD section 2 with its SHA-256; a flavour is never delivered on a unit (§8.3). The cases that need them are TC-SW-KEYER-019, 034, 035 and 038 and TC-TX-003 and 014.
...
| LTspice (TV due PDR) | `tools/tests/fixtures/ltspice/rc-lowpass.asc` run as `/Applications/LTspice.app/Contents/SharedSupport/ltspice/bin/wine --bottle=ltspice --wait-children 'C:\Program Files\ADI\LTspice\LTspice.exe' -b <deck>` (the headless form of `docs/research/ltspice-batch-macos.md` F3; the app launcher `Contents/MacOS/LTspice -b` exits 0 with no output, F2, and is not permitted) reproduces the stored -3 dB frequency within 1 %; the `.log` first line names the version; the seeded-error netlist `tools/tests/fixtures/ltspice/rc-seeded-error.net` makes the wrapper exit 1 (F6); an `.asc` with a deck error hangs with no log (`rc-hang-error.asc`), so decks for the record are netlists and every run has a time-out guard that kills only its own processes (`tools/toolchain.lock.md` §1.4 items 1 and 2). Precondition: `CaptureAnalytics=false` in the bottle's UTF-16LE `LTspice.ini`, checked as `iconv -f UTF-16LE -t UTF-8 "$INI" | /usr/bin/grep -q '^CaptureAnalytics=false$'` and never appended to (F4, SI-027). |
| OpenSCAD + FreeCAD with `tools/scad2step.py` (TV due PDR) | CSG export of `tools/tests/fixtures/openscad/cube.scad` with OpenSCAD 2021.01 to an absolute path, then `freecadcmd tools/scad2step.py` gives a STEP with one valid solid and the stored bounding box. `tools/scad2step.py` unwraps a `Compound` of exactly one solid (FreeCAD 1.1.3 returns the OpenSCAD cut that way) and prints a success marker; the caller checks the marker and the STEP file, never the `freecadcmd` exit status, which is 0 even when the script raises (`tools/toolchain.lock.md` §1.4 item 4). |
...
| `tools/render_review_figures.py` (TV due SRR) | `-p 'test_render_review_figures.py' -k ParserTests -k DataKnownAnswerTests -k RunTests` passes on the fixture `tools/tests/fixtures/review_figures/`: every drawn value equals the hand-computed answer, each seeded disagreement stops the run with exit 1 and writes nothing, and every render has the stored pixel size. Its `RepositoryTests` class checks repository content and is recorded separately (as for `tools/traceability.py` above). |
...
| AL-10 | `docs/process/02-requirements-and-traceability.md` §10.3 item 4 and 02 AL-8 | Risk, software classification and RMM, and ConOps impacts written into other rows until 05 §5.3 gains them | §5.3 now has the rows Risk, Software classification and tailoring, Operations and ConOps, and Cybersecurity; the CR template §4 has the same rows | Done 2026-09-25 (verified by the 05 author): 02 section 10.3 item 4 names the 05 section 5.3 rows Risk, Software classification and tailoring, Operations and ConOps, Cybersecurity and Regulatory and says the CR template section 4 has the same rows; 02 AL-02-08 is Closed and states that item 4 points to those rows. |
...
| AL-12 | `docs/plan/semp.md` §5.14 | Baselines listed without the customizations | §1 customizations 1 and 2; the single CR trigger (CR-from event); product waivers | Done 2026-09-25 (verified by the 05 author): SEMP §5.14 states both customizations (functional baseline at SRR instead of SDR; fourth baseline as-built at SAR instead of as-deployed at ORR) and cites 05 §2 for product waivers; the SEMP customization table rows 4 and 5 cite 05 §1 items 2 and 1. |
...
| AL-14 | Repository root (untracked) | `result.json` (a Bambu Studio CLI result) and `reveal.js/` (a reveal.js copy written outside any deck folder) | Match no Table 4-1 row | Never committed. Prevention done 2026-09-25: `tools/slides/render_deck.py` exits 2, writing nothing, for a deck outside `docs/reviews/<REVIEW>/slides/` (known-answer test `tools/tests/test_render_deck.py` class `LocationGuard`), so no new stray `reveal.js/` copy can be made. Removal done: on 2026-09-25 neither `result.json` nor `reveal.js` exists at the repository root (`ls result.json reveal.js` exit 1, "No such file or directory" for both, run by the 05 author). |
| AL-15 | `docs/process/rmm.json` rows SWE-082 and SWE-085 (implementation texts) | SWE-082: levels of control "working tree, review-ready, baselined". SWE-085: `firmware/releases/` "holds the UF2 image, its SHA-256 and the VDD"; flashing follows the operations handbook | §4.1 levels L0 Working, L1 Reviewed, L2 Controlled, L3 Released; §8.1 step 4 writes the ELF, UF2, `firmware.map`, `picotool-info.txt` and `SHA256SUMS` to `firmware/releases/vX.Y.Z/` with the VDD beside it; §8.3 holds the flashing and delivery procedure | Closed 2026-09-26 (INSP-006 finding-7): both `rmm.json` implementation texts now name the §4.1 levels and the §8.1 step 4 and §8.3 procedures; `tools/render_rmm.py --check` exits 0. |
...
| SWE-045 (participate in joint NASA/developer audits) | Fully compliant, as the RMM row SWE-045 (NPR 7150.2D §5.1.9, disposition FC) records: the owner, in the acquirer role, takes part in the FCA and PCA at SAR (§7.1, §7.2, recorded in `docs/reviews/SAR/configuration-audit.md`) and performs the hands-on PCA steps (§3 owner row, §7.2). |
