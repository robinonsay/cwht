...
**Anchor availability and the band argument.** Anchor 1 needs a Test on the delivered unit or a heritage design confirmed by two analyses, and anchor 2 needs an analysis made with a tool that holds an accepted `TV-NNN` record (SWE-136, 05 section 9.2). On 2026-09-25 neither exists (no delivered unit; `docs/cm/tool-validation/` holds evidence logs but no accepted TV record), and some risks have no design analysis at all. Every likelihood rationale therefore names one of the three cases below, and the reviewer checks it at section 16 item 4:

| Case | Applies to | Level rule | Confidence |
|---|---|---|---|
| (a) Anchor | A design-analysis risk: the departure is a property of the designed product that an analysis or a test can bound | The level is the anchor that the facts in the rationale meet: no analysis gives 4; one analysis with unconfirmed tool or model fidelity, a margin below 2x, or a design that deviates from every reference design gives 3. A lower level is recorded only under case (b) | As assessed |
| (b) Band argument over an anchor | A design-analysis risk whose facts meet anchor 3 or 4, where named evidence outside the project's own analysis places the probability in a lower band (a reference design used without change, a database of certified products, a vendor measurement, an operating history) | The rationale begins "Band argument", names the evidence, says why the anchor does not decide, and records the level of the band | Medium at most; Low when the evidence is secondary |
| (c) Band argument, no anchor | A threat, procedural, workmanship, vendor-part, regulatory-interpretation, operating-circumstance or toolchain risk: the departure depends on a person's act, a vendor, a regulator or a process step, and no design analysis bounds it | The rationale begins "Band argument", names the case and the evidence (for example the 07 section 16.2 threat assessment, vendor errata, the project's own record) and records the level of the band | Medium at most until an observation (test, audit, trigger poll) supports it |

Case (a) is the default: a rationale that names neither band argument is read as case (a), and the anchor decides.
...
| Yellow | At least one trigger; strategy *Mitigate*, *Research* or *Elevate* with steps, or *Watch*, or *Accept* under the strategy table below | Claude plans; owner sees the list at the review | Listed at every review; discussed on request |
| Green | At least one trigger; strategy *Watch* with closure criteria, *Mitigate* with steps, or *Accept* under the strategy table below | Claude | Listed at every review |

**Strategies.** The `mitigation.strategy` values of `docs/risk/schema.json`:

| Strategy | Definition | Bands allowed while the risk is active | Requires |
|---|---|---|---|
| *Mitigate* | Reduce likelihood or consequence by steps that change the design, the verification or the process | Any | Steps with action, artifact, due gate and status (Red: at least two active), triggers and a fallback |
| *Research* | Reduce the uncertainty of the assessment before a mitigation is chosen; the preferred strategy for a Low-confidence risk (section 3) | Any | At least one step whose output is the analysis, measurement or study that re-assesses the risk, with its due gate; a trigger that re-assesses the risk on that output; a fallback |
| *Elevate* | The response is outside Claude's authority: it needs the owner's decision on money, a purchase, a baseline or the scope | Any | A step whose artifact is the decision memo, ADR or CR that carries the owner's decision, due at the next gate or earlier; a trigger for a decline; a fallback |
| *Watch* | No step that changes the design or the process: the risk is carried by its triggers, and any steps are confirmation steps (a measurement, an inspection or a record) that test the assessment without reducing it | Yellow and Green only (never Red; section 9 transitions) | Triggers with actor and timing; closure criteria. Status *Watch* (section 9) further needs the confirmation steps Done or Dropped |
| *Accept* | The plan proposes that the owner accept the residual at a named gate | Yellow and Green only (a Red risk reaches *Accepted* through a *Mitigate*, *Research* or *Elevate* plan and the owner's acceptance record) | Before the decision: a step due at the named gate whose artifact is the decision memo, ADR or CR that records the ruling, and the package decision or ADR that asks for it; after the decision, status *Accepted* with the acceptance record below, or on a decline a new plan with another strategy and a `history` entry |
...
|---|---|---|---|---|---|
| Identify | Write the four-part statement and title; take the next free `RSK-NNN` (never reused, charter §6); set `status: Proposed`, `source`, `opened`, `horizon`, `category`, `tags`. Dispose of every candidate risk named in another artifact (disposition rule below) | Stakeholder inputs `SI-NNN`, requirements and TBR list, hazard analysis, ConOps section 8, research reports in `docs/research/`, ADR section 4.4 risk fields, trade studies, plan risk tables (07 sections 16.2 and 21), simulation and analysis reports, vendor DFM reports, tool validation results, RFAs and RIDs, NCRs, lessons learned, review discussions | New entry in `docs/risk/register.json`; `candidates` entries | Claude; any agent may propose a risk in its structured result | Continuously; formally at every review and on every trigger |
...
| Plan | Choose strategy (section 8 strategy table); write steps (action, artifact, due gate, status), triggers (observable condition, response with actor and timing), fallback, closure criteria | Band table of section 8; cost model; milestone list | `mitigation` object; for Red, `plan_approval` after the owner approves | Claude; owner approval for Red plans | Before the next review |
...
**Candidate disposition rule (Identify).** Every candidate risk named in another artifact receives exactly one disposition in the `candidates` array of `docs/risk/register.json`. The source artifacts are: ConOps section 8; the risk lists of `docs/research/*.md`; the hazard analysis register-linkage table; the "Risks opened, closed or re-scored" field (section 4.4) of every ADR; the "Related risks" row of every trade study, and on its decision day the per-alternative risks of the selected alternative (section 13 item 3); the risk tables and risk-opening rules of the plan documents (07 sections 16.2 and 21; the watch items of `docs/plan/technology-assessment.md` section 7; any SEMP, 01 to 05 or `docs/plan/tpm.json` rule that says a condition opens a risk); review minutes; RFA answers. The dispositions are *Entered* (it became a new `RSK-NNN`), *Merged* (it is folded into an existing `RSK-NNN` whose `source` names the candidate id, with a rationale and a `history` note on that risk), or *Declined* (not a risk, with a rationale of at least 20 characters that names the condition that would re-open it). The candidate keeps the id its source artifact gives it; a candidate whose source gives no id takes `<SRC><section>-<TOPIC>` (for example `SEP16-KEY` for the 07 section 16.2 keying row, `ADR011-R1` for the first ADR-011 candidate). `tools/render_risk.py` checks that every Entered or Merged candidate names an existing risk whose `source` contains the candidate id, and renders the dispositions.

Status (2026-09-25): the ConOps section 8 pass (17 candidates: 6 Entered, 8 Merged, 3 Declined) and the research and hazard-analysis passes are complete; the plan-table pass over 07 sections 16.2 and 21 (15 candidates, all Entered, 4 of them as the new RSK-060 to RSK-063) and the ADR pass over ADR-001 to ADR-026 (14 candidates: 3 Entered as RSK-063 to RSK-065, 8 Merged, 3 Declined) were run after INSP-007 findings F-03 and F-13; the technology assessment section 7 watch items all name existing risks (RSK-003, RSK-005, RSK-010, RSK-013, RSK-044). TS-001 and TS-002 are undecided, so their per-alternative risks are dispositioned on their decision days. Claude runs the pass over the remaining plan documents (SEMP, 01 to 05, `tpm.json` conventions) before the SRR readiness declaration (`01-lifecycle-and-reviews.md` section 3.1), and repeats the pass for every new research report, ADR or trade study decision the day it is committed.
...
**Measure thresholds and responses.** Claude analyzes the measures table in the risk section of every review package against these thresholds; the reviewer confirms the analysis with the measures table it records at section 16 item 11.

| Measure | Threshold | Response |
|---|---|---|
| Open steps overdue (due at an earlier gate) | Any at a gate | Each is a RID against the register in `docs/reviews/<REVIEW>/rfa-rid-log.json`: Major for a step of a Red risk or of a risk carrying a hazard, Minor otherwise, unless a `history` entry dated before the readiness declaration re-plans the step with a reason |
| Open steps due at the gate being prepared | Any open at the readiness declaration | Closed or re-planned with a `history` entry before the declaration, except a step whose output is a ruling of that review's board; the package lists each such step with the decision number that closes it |
| Red count | Higher than at the previous gate | The package names each risk that became Red and the cause (new risk, re-score, trigger, review finding) |
| Red risks still active | Any at CDR | Section 8 Red row: CDR disposition at most *Approved with liens* unless the owner records acceptance |
| Opened since the previous gate | Any risk whose source artifact existed before the previous gate's candidate pass | The package names the missed source; Claude adds a `docs/lessons-learned.md` entry and extends the section 9 source list if the source type was not on it |
| Closed or retired | Any | Listed with the decision memo entry; a closure without its closure-criteria evidence is a Major reviewer finding |
| Accepted | Any | Listed with residual score and acceptance record; the tool rejects an acceptance without one |
...
3. **Register update.** When the `TS-NNN` file is created, every risk its "Related risks" row names gets `related.trade_study_ids` with that id and a `history` entry naming it (done for TS-001 and TS-002 on 2026-09-25). On the day the decision is recorded (charter §11 rule 6) the risks of the selected alternative are entered into the register or merged into existing risks with `related.trade_study_ids`, and every linked risk is re-assessed. A mitigation step whose study is not yet written names it `TS-NNN-<slug>` with "number assigned at creation"; a trade-study number is taken only when its file is created, because a number assigned in advance can end up naming another study (the concept section 11.2 number TS-002 for the synthesizer study now names the firmware make/buy study) and charter §6 forbids reusing an id.
...
| NPR requirement | NPR 7150.2D SWE-086 (§5.2.1, all software risks and mitigation plans); RMM disposition FC (Fully Compliant) | Section 5 (software risk = category software or tag software, enforced by the tool), section 9 (six steps, SWE-086 record row of `register.md`; the candidate rule's plan-table and ADR sources, which bring in every software risk that 07 and the ADRs name), section 8 (residual and accepted risks stay rendered), section 16 item 11 (software assurance audit) |
| NPR requirement | NPR 7150.2D SWE-154 (§3.11.3, cybersecurity risks and mitigations); RMM disposition T (Tailored: flight and ground software reduce to the firmware and host-side tools; Project Protection Plan provisions not applicable; charter §12 scopes the assessment to USB firmware loading and command injection via the key input) | Cyber risks and their mitigations are `RSK` entries tagged `cyber`, one per 07 section 16.2 row: RSK-015 (firmware image and persisted configuration, the USB loading surface), RSK-022 (debug access), RSK-060 (command injection via the key and control inputs, the charter §12 surface), RSK-061 (diagnostic interface), RSK-062 (build and supply chain); mitigations planned as `REQ-SW-*` and `TC-SW-*` steps; 07 section 16.2 is the assessment. The RMM row's implementation text names the first three surfaces only (cross item to the RMM author) |
...
4. Every likelihood rationale names its anchor or its band-argument case (section 6, anchor availability) and meets that case's level rule; every consequence rationale names the driving dimension.
...
| Software risks "opened in `docs/risk/register.json` at SRR by the software lead" | 07 section 21 | Updated 2026-09-25 (INSP-007 F-03, F-10): 07 section 21 has nine rows and all nine are in the register: 'MC/DC tooling gap' RSK-010, 'Emulator fidelity' RSK-003, 'Complexity tool staleness' RSK-019, 'rustos driver effort' RSK-013, 'Toolchain not qualified' RSK-020, 'Flash write path' RSK-021, 'Wrong image or configuration accepted' RSK-015, 'Host mock diverges from silicon' RSK-063 (opened 2026-09-25), 'Single maintainer of rustos' RSK-023; candidates SEP21-MCDC to SEP21-MAINT record the dispositions. No difference. |
| Cyber risks of the 07 section 16.2 assessment in the register | RMM rows SWE-154 and SWE-156; 07 section 16.2 (six rows); charter §12 (command injection via the key input) | Updated 2026-09-25 (INSP-007 F-03, F-15): one `cyber`-tagged risk per row: image integrity and persisted configuration RSK-015, keying and control inputs RSK-060, debug access RSK-022, diagnostic interface RSK-061, build and supply chain RSK-062; candidates SEP16-IMG to SEP16-SUPPLY. Difference: the RMM SWE-154 implementation text lists image tampering, malformed configuration and debug port access only; action for the RMM author before the SRR readiness declaration: add the key-input injection, diagnostic-interface and supply-chain surfaces with RSK-060 to RSK-062. |
| "The corpus is complete (267 pages)"; "Risk owner closes RSK-009 at SRR" | 07 section 22 | Superseded by the lead SE decision in RSK-009's history (2026-09-25): RSK-009 stays open to its PDR horizon; S1 is Done (reproduced by INSP-007) and S2 to S4 are due PDR, so its closure criteria are not met at SRR. Action: 07 author rewords the row to "RSK-009 closes on its closure criteria (target PDR)" in 07's next revision, and the package author rewords package decision 14 to keep RSK-009 open (INSP-007 F-09), both before the SRR readiness declaration. |
| SWEHB page count | Charter §1 row NASA-HDBK-2203; `08-agent-briefing.md` section 1 repo map | Updated 2026-09-25 (INSP-007 F-10): charter §1 now reads "267 files, 130 of them SWE pages", which matches the directory (RSK-009 condition); the earlier charter issue is resolved. Remaining difference: the 08 section 1 repo map still says "268 pages scraped"; action for the 08 author, editorial change before the SRR readiness declaration. |
| Hazard `related_risk_ids` back-links | `docs/safety/hazards.json`; `docs/safety/hazard-analysis.md` section 12 | Updated 2026-09-25 (INSP-007 F-01, F-08, F-10): every back-link action listed in the previous revision of this row is done, HZ-015 [RSK-034] included (read from `hazards.json` on 2026-09-25). One new action: RSK-046 now names HZ-008 (the C7 wrong-frequency-word cause, safety 4 against Critical), so the hazard analysis author adds RSK-046 to HZ-008 `related_risk_ids`; until then `render_risk.py --check --gate SRR --hazards` exits 1 on that back-link only. RSK-028 is not linked to HZ-006 (decision recorded in RSK-028 history: RSK-016 carries HZ-006). |
