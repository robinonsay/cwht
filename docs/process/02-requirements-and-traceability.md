...
| T-03 | yes | `ID_DUPLICATE` (REQ, TC, NCR across and within files; NGO, MOE, CON within `expectations.json`); stakeholder `name` uniqueness through `STAKEHOLDERS_MISSING` (T-21) | none | done (2026-09-26) |
...
| T-18 | yes (SRR W listing) | `SYS_UNALLOCATED`, reading `child_ids` and the preliminary `allocation.json` (W) | the E at PDR comes with `--gate` | done at SRR (W); PDR (E) |
...
| T-21 | yes | `EXPECTATIONS_INCONSISTENT` (Need count, parent kinds, `ngo_ids`, `ops_ids`); `retired_by` pattern by T-01; `STAKEHOLDERS_MISSING` (array absent, required roles missing, names not unique, sources unresolved); stakeholders rendered by `--render` as section 7 of `expectations.md` | none | done (2026-09-26) |
...
Raised by the author of this document for the integrating session (section 8.6). Charter commit b3d2e13 closed CI-1, CI-3 and CI-5; charter commit b8214ca closed CI-2, CI-4 and CI-7 and adopted the Verified-versus-Closed rule for host-verified software that AL-02-17 had argued against; commit 4e3f891 closed CI-9. CI-10 is the one open charter item: an owner choice (SRR package decision 103) that needs a charter edit with a `Refs:` trailer before SRR only if the owner adds a scheme, since the charter is Log-class until SRR (05 Table 4-1 row 1). Alignment items carry the prefix `AL-02-` because 05 section 14.1 already uses `AL-1` to `AL-14` for other items; the number after the prefix keeps the number this document used before (AL-7 is now AL-02-07).

**Status refresh, 2026-09-25** (SRR package `docs/reviews/SRR/package.md` sections 2 item H16, 15 item 20 and 18.2). Every row was re-checked by reading the named file at HEAD 28e49e6. The process documents 01 to 08, `docs/process/README.md`, `docs/process/rmm.json`, `docs/templates/`, `tools/README.md` and `docs/plan/tpm.json` entered version control in commit 4e3f891, and the `stakeholders` array of `expectations.json` in 28e49e6, so each row that read "closed in the working tree; commit pending" now names the commit that confirms its closure. "Closed in the working tree" now applies only to AL-02-25 and AL-02-26, edited on 2026-09-25 after 28e49e6; their closure is confirmed by the commit that records those edits (charter section 11 rule 2), which the SRR package section 18.2 lists.
...
| CI-7 | Charter section 5 | The per-review working files listed both `checklists/` and `peer-reviews/INSP-NNN.md`. | Closed by b8214ca: the filled checklist is the single record with `id: INSP-NNN`, no `peer-reviews/` folder (section 5). The other documents that cited `peer-reviews/` were AL-02-10, whose path part is closed. |
...
| CI-9 | Charter section 6 | The module list "SYS, RX, TX, PWR, CTL, ME, SW-<sub>" omits `SW`, the firmware-wide L2 module that charter sections 5 (`docs/requirements/{rx,tx,pwr,ctl,me,sw}/requirements.json`) and 7 (L2 includes SW) use and that section 2.2 and 07 section 4 item 1 define. | Closed by 4e3f891 (verified 2026-09-25): charter section 6 lists `SW` for firmware-wide software requirements in the `REQ-<MOD>-NNN` module list and in the requirement-module sentence (`docs/requirements/sw/requirements.json`). |
| CI-10 | Charter section 6 | No identifier scheme exists for stakeholders; section 3.0 keys stakeholder entries by unique `name`. | Open, owner's choice before the SRR readiness declaration (SRR package decision 103; recommendation and default: keep the `name` keys, no change): keep `name` keys, or add a scheme such as `STK-NNN` to charter section 6, after which a schema edit adds an `id` field to the `stakeholder` definition. |
| AL-02-07 | 04 sections 7.1 and 7.3 rule 2 | "until the schema gains that value no requirement may be retired" (04 section 7.1, working tree 2026-09-25). | Closed by 4e3f891 (verified 2026-09-25): the quoted sentence is gone from 04, and 04 sections 5.3, 7.1 and 7.3 rule 2 use the interim marker of section 11.3 (requirement status `Closed` with tag `retired`; test case `Blocked` with `setup` starting `Retired by `), which the tool recognizes. |
| AL-02-08 | 05 section 5.3, `docs/templates/change-request.md` | No impact rows for software classification and RMM, risk, ConOps. | Closed by 4e3f891 (05 section 5.3 and CR template section 4 read 2026-09-25 have the rows Risk, Software classification and tailoring, Operations and ConOps, and Cybersecurity; 05 section 14.1 AL-10); edits by the 05 author. Section 10.3 item 4 now points to those rows. |
| AL-02-09 | 05 section 5.4, 07 MSR-02, RMM SWE-200 | Volatility computed by another script and counting rule. | Closed by 4e3f891 (05 section 5.4, 07 section 11.2 row MSR-02 and RMM row SWE-200 read 2026-09-25 name `tools/traceability.py --volatility` and the section 10.4 rule); edits by the 05, 07 and 03 authors. |
| AL-02-10 | 07 sections 1.2 (Records row), 3.4, 10.2 and 22; `docs/templates/peer-review-checklist-requirements.md` "Record front matter" (line 9 and the `checklist_file` line) | These still name `docs/reviews/<REVIEW>/peer-reviews/INSP-NNN.md` as the peer-review record (working tree 2026-09-25, 18:20). Charter section 5 (b8214ca) makes `docs/reviews/<REVIEW>/checklists/<product-slug>.md` with `id: INSP-NNN` the single record with no `peer-reviews/` folder, and `tools/validate_docs.py` rejects such a folder. | Closed 2026-09-26. Path part closed by 4e3f891 (verified 2026-09-25). Residual closed: `docs/templates/peer-review-checklist-requirements.md` revision C carries the per-requirement validation table of section 5 (columns WR failures, V1 to V6, CK-REQ items answered No, disposition). |
| AL-02-11 | 03 section 8, 07 CS-24 | 03 traced code with `// Trace:` comments. | Closed by 4e3f891 (03 section 8 uses the CS-24 tags and the 07 paths); edit by the 03 author. |
| AL-02-12 | 07 section 4 | Called `sw/sw-<sub>/` files "L3". | Closed by 4e3f891 (verified 2026-09-25): 07 sections 1.3, 3.1, 3.5 and 4 no longer say L3, and the former residual is done: 07 section 11.2 row MSR-04 reads "by CDR (L2, including the module files)". |
| AL-02-13 | `docs/process/README.md` | Claimed SE-09 for this document. | Closed by 4e3f891 (SE-09 on the SEMP row). |
| AL-02-14 | `docs/plan/semp.md` sections 5.2 and 7.4, `docs/plan/tpm.json` | No `mops[]`; the SEMP did not define MOPs. | Closed by 4e3f891: `tpm.json` holds MOP-001 to MOP-020 with every performance TPM naming its MOP, and SEMP section 7.4 names `mops[]` (section 6.2); the SRR values added in 28e49e6 keep the 20 MOPs. |
| AL-02-15 | 08 sections 3.1 and 3.5 | ICD reviewer checklist named a non-existent file. | Closed by 4e3f891: 08 names `peer-review-checklist-design.md`, which has sections I and J. |
| AL-02-16 | 05 section 4.3 identification table; 03 section 8 | Retired status and `control_req_ids` contradicted the charter. | Closed by 4e3f891: 05 section 4.3 writes status `Closed` with tag `retired`; 03 section 8 names the union of the per-control lists. |
...
| AL-02-18 | 04 sections 5.2 (platform-independent row), 5.3 (requirement status), 7.3 rule 4 and 7.4 row 7.3.4 | 04 made `Verified` for `REQ-SW-*` require the PCA-05 line and named the check `VERIFIED_NOT_INSTALLED`. 05 section 14.1 AL-11 raised the same item. | Closed by 4e3f891 (verified 2026-09-25 by the integrating session): 04 sections 5.2 and 5.3 put the PCA-05 line and the signed SAR memo under `Closed` per charter section 9, and 02 (T-11), 04, 05 and `tools/README.md` use the single code name `CLOSED_NOT_INSTALLED` (implementation due CDR). |
| AL-02-19 | RMM row SWE-051 (`docs/process/rmm.json`) | Says ICDs are cited "via source_ids"; section 3.4 cites interfaces in `design_refs`. | Closed 2026-09-26: `rmm.json` row SWE-051 reads "via design_refs; 02 section 3.4" and `tools/render_rmm.py --check` exits 0 (checked by Claude as integrator). |
| AL-02-20 | 07 section 4 item 2 | Module-file requirements carry "source_ids naming the hazard (HZ-NNN), ICD, ADR or SWE-134 item". Section 3.4: hazards go in `hazard_ids`, ICDs in `design_refs`, only `ADR-`/`TS-` in `source_ids`; a SWE-134 item is named in `rationale`. | Open (re-checked 2026-09-25: 07 section 4 item 2 still reads so), owner: 07 author, before PDR (first module-file requirements). |
| AL-02-21 | `docs/requirements/l0-stakeholder/expectations.json` | Has no `stakeholders` array (section 3.0; schema definition `stakeholder`). | Array part closed by 28e49e6: `expectations.json` holds the `stakeholders` array (eight entries transcribed from section 3.0; `tools/validate_docs.py` passes). Open, owner: Claude as tool maintainer, before the SRR readiness declaration: the rule `STAKEHOLDERS_MISSING` (T-21) and the `--render` stakeholders section of `tools/traceability.py` (section 8.5). Neither is in 28e49e6; an uncommitted edit adding both was in progress in the working tree when this row was refreshed, with three known-answer tests of `tools/tests/test_traceability.py` failing against it. The row closes when that edit passes the full `tools/tests` suite and is committed; until then the SRR package applies T-21 by hand (package section 2 item H4 and section 8). |
| AL-02-22 | `expectations.json` or `docs/decisions/adr/` | No `CON-` or `ADR-` records the cybersecurity assessment scope of charter section 12, which the tag `security` requires as a source (section 2.3). | Open, owner: Claude, before the first `security`-tagged requirement is written: a `CON-` of kind `Process` with source SI-015 (Class A process election) stating the charter section 12 scope, added to `expectations.json`. Re-checked 2026-09-25: no requirement file carries the tag `security` yet, so the trigger has not fired. |
| AL-02-23 | 05 sections 2, 5.2 and 6 (CSA item 12); 04 section 10.4 `Use-as-is` row | Revision C had no waiver path. | Closed by 4e3f891: 05 section 2 defines the product waiver (a CR with disposition `Approved (waiver)` or a memo item `W<n>`), section 5.2 the disposition and CSA item 12 the waiver register; 04 section 10.4 `Use-as-is` already requires a CR that waives the requirement. Section 8.7 follows them; no further edit needed. |
| AL-02-24 | `docs/templates/icd.md` sections 3.2.5 and 3.2.6 | No `Security expectations` row (section 3.5 Content). | Closed 2026-09-26: `docs/templates/icd.md` has the Security expectations rows in sections 3.2.5 and 3.2.6; ICD-CTL-KEY fills 3.2.5, ICD-CTL-USB fills 3.2.6, and the other stubs mark both rows Not applicable with a reason (INSP-012). |
| AL-02-25 | 01 section 4.3 row 28 | Names `tools/refs/` as the conversion of the regulatory corpus; the corpus was fetched with the eCFR versioner API command of `docs/references/md/regulatory/README.md` (section 3.4). | Closed in the working tree 2026-09-25: 01 section 4.3 row 28 names `docs/references/md/regulatory/` with the provenance of its `README.md` (Parts 1, 2, 15 and 97 verbatim at eCFR issue 2026-09-23, fetched with the eCFR versioner API command recorded there, plus the FCC extracts), and 01 no longer names `tools/refs/`, which holds only the NASA corpus scripts. Confirmed by the commit that records the edit. |
| AL-02-26 | `docs/plan/semp.md` customization list | The single-valued `priority` of section 6.4 is a customization of SE HB §4.2.1.2.3. | Closed in the working tree 2026-09-25: SEMP section 9.0 now holds the single customization list (NPR 7123.1D section 2.2.2.2), whose row 10 is the single-valued `priority`, and SEMP section 5.2 points to it; the two CM baseline customizations of 05 section 1 that 05 section 14.1 AL-12 asked for are rows 4 and 5 of the same list (SEMP section 5.14 already mirrored them). Confirmed by the commit that records the edit. |
