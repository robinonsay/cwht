...
**Status:** Draft for SRR (functional baseline candidate), revision 2 of 2026-09-25, updated for peer review record INSP-002 (`docs/reviews/SRR/checklists/conops-and-concept.md`). **Owner:** Robin (customer, licensee, Decision Authority). **Author:** Claude (lead systems engineer). **Governing text:** SE HB §4.1.1.2.4 (establish concept of operations and support strategies), SE HB App. S (annotated outline; section map below), NPR 7123.1D SE-36 (concept definition, an SRR minimum product per `docs/process/01-lifecycle-and-reviews.md` section 4.5), charter `docs/process/00-charter.md` sections 5, 7 and 9, and `docs/process/02-requirements-and-traceability.md` section 3.3 (scenario headings `### OPS-NNN <title>`, each with actors, preconditions, steps, expected outcome and the MOEs it exercises).
...
Robin, a US General-class amateur licensee (SI-014), wants to "play radio" with a small circle of licensed friends (SI-019, SI-030) by Morse code on the 2 m band, from pockets, parks and hilltops, using true continuous-wave keying rather than the tone-modulated FM that every commercial 2 m handheld offers (SI-004). A narrow-band A1A signal is received in a 500 Hz passband and needs about 13 dB less signal than 12 dB SINAD FM, which is roughly twice the range over ground or four times in free space (`docs/research/antenna-and-erp.md` F11). The reference-design survey found no pocket 2 m CW handheld with a built-in keyer for both a straight key and iambic paddles (SI-018; `docs/research/2m-cw-transceiver-reference-designs.md` F1 to F5); the nearest precedent is the historical Mizuho MX-2, a 200 mW SSB and CW handheld with no keyer (F3). So the owner designs and builds one: revision A of the cwht ("CW handheld transceiver").
...
| `docs/safety/hazards.json` (version 0.3.0-pha), `docs/safety/hazard-analysis.md` | HZ-001 to HZ-015 and their phases (mapped to the modes in section 3.4) |
| `docs/plan/tpm.json` | MOP-001 to MOP-020 and TPM-001 to TPM-020 (TPM-006 temperature span, TPM-008 duty cycle) |
| `docs/risk/register.json` | RSK-001 to RSK-059 |
...
| **Transmit-keyed** | T10 | Receive audio fades out; antenna to transmit; after the first-element lead-in, the shaped carrier rises at the selected step; the sidetone follows the key; the hang timer restarts at every key-up; the key-down timeouts run (straight key 5 s, paddle 128 consecutive identical elements or 10 s of them in elapsed time (TBR, REQ-SYS-054; 30 s is the alternative of SRR package decision 37, D-KN3)); the independent key-down cutoff bounds any continuous key-down at 10 s (7.5 to 13 s); TX indicator and key-down time shown | T11 hang time expires after the last key-up, or at once when any flag or inhibit arises (the carrier ends through the shaped fall, reaching the RF-off level of REQ-SYS-183 within 20 ms (TBR, REQ-SYS-004) of detection, and the antenna returns to receive without waiting for the hang); T17 to T20 |
...
| LOWBATT | Either cell below the transmit cutoff of 3.20 V +/-0.05 V (TBR; 6.4 V for a balanced pack), measured in receive (REQ-SYS-097) | Per-cell measurement | Pack back above the cutoff plus a hysteresis band (charged or new cells) | Re-evaluated at every Self-test | "TX off: low batt" shown; receive continues (REQ-SYS-097) |
...
| 3 | Paddle: 128 consecutive identical elements or 10 s (TBR) of them, elapsed time from the first of those elements, whichever comes first (REQ-SYS-054; 30 s is the alternative pending D-KN3, package decision 37) | Inhibit (KEY) | "PADDLE?" | Both paddle contacts are confirmed open | Continues while the paddle is held | As row 2 | Unaffected |
...
| 5 | Either cell below 3.20 V +/-0.05 V (TBR; 6.4 V for a balanced pack; REQ-SYS-097) | Flag LOWBATT | "TX off: low batt" | Table 3.4-3 | Sidetone rule | On | Not applicable |
...
- F7. No Tune carrier longer than 5.5 s (*proposed*) and no continuous key-down from any single cause longer than 13 s; a toggling key stream is bounded by the firmware paddle watchdog (OPS-013 steps 3 and 6) and, if package decision 38 adopts it, by the 150 s to 180 s transmission-length backstop of REQ-SYS-180.
...
|---|---|---|
| Off | Storage, Handling (Assembly during kit assembly) | HZ-007, HZ-009, HZ-010 (ESD in handling), HZ-013, HZ-015 (owner hand assembly) |
...
1. Tune and listen anywhere in 144.000 to 148.000 MHz with a 500 Hz CW passband, a CW pitch set by the operator as the sidetone frequency (300 to 1000 Hz in 10 Hz steps, item 4; REQ-SYS-045, REQ-SYS-046), and a tuning step that follows knob speed (*proposed* selectivity set, NGO-012). The BFO offset range of +/-500 Hz in 10 Hz steps is a per-unit calibration stored at acceptance (section 3.5.3; REQ-SYS-144), not an operator control.
...
4. Semi break-in with an operator-set hang time of 3 to 30 dits at the displayed speed, 8 dits by default (REQ-SYS-044, ADR-026) and a sidetone of 600 Hz by default (300 to 1000 Hz) locked to the receive offset (NGO-014, CON-021).
...
6. Store the operator's call sign and show it at every Self-test; show an identification reminder 9 min 00 s +/-5 s (TBR) after the first transmission following the previous reminder, one minute ahead of the 10 minute interval of 47 CFR 97.119(a) (NGO-020, CON-005, REQ-SYS-068). Revision A sends no automatic identification: the operator keys every identification (section 3.5.4; REQ-SYS-007; Appendix C automatic-ID row).
...
10. Safety: key-closed interlock, straight-key 5 s timeout, paddle watchdog (128 consecutive identical elements or 10 s of them in elapsed time (TBR, REQ-SYS-054; 30 s is the alternative of SRR package decision 37, D-KN3)), a key-down cutoff independent of firmware at 10 s (7.5 to 13 s), over-temperature inhibit, dual-path cell sensing, protected pack, hardware transmit inhibit while USB power is present, audio ceiling and default cap, and the inhibit and latched classes of Table 3.4-4 (NGO-016, NGO-021, NGO-022).
...
| Handling and thermal | Enclosure hand-hold area stays comfortable to hold after the SI-034 duty cycle; at the 5 W step the final stage dissipates about 3.3 W at the TPM-004 planned drain efficiency of 60 percent (5 W / 0.60 - 5 W); the driver and bias add to it, and the 4 to 7 W that `docs/research/keyer-verification-and-key-input-network.md` F13 item 4 uses for the whole amplifier chain corresponds to a line-up efficiency of about 42 to 56 percent; the PA thermal analysis at PDR replaces both figures with the selected device's; so continuous key-down is bounded by the 13 s independent cutoff and by the over-temperature inhibit | SEMP section 7.3.1 handling row; RSK-006, RSK-026; `docs/research/keyer-verification-and-key-input-network.md` F13 item 4 |
...
3. Tunes so the wanted signal sits at the sidetone pitch, which is the receive pitch (REQ-SYS-046; zero-beat means netted), and changes the sidetone frequency if a different pitch is wanted; adjusts volume; the volume ramps without zipper noise and never exceeds the cap.
...
4. The friend answers; the operator copies the reply through the 500 Hz passband, sends a signal report and a short exchange, identifies with their own call sign at the end and at least every 10 minutes (47 CFR 97.119(a)); the display's identification reminder appears 9 min after the first transmission (REQ-SYS-068).
...
**Actors:** the operator.
**Preconditions:** a day of operating at about 1:9 transmit-to-receive; cells approaching 3.20 V each (6.4 V for a balanced pack).
**Steps:**
...
2. When either cell, measured in receive, reads below the transmit cutoff of 3.20 V +/-0.05 V (TBR, REQ-SYS-097; 6.4 V for a balanced pack) the LOWBATT flag sets: transmit is inhibited, receive continues, and the display says "TX off: low batt"; a carrier in progress ends through the shaped fall.
...
3. *Paddle stuck or held:* after 128 consecutive identical elements or 10 s (TBR) of them, whichever comes first (REQ-SYS-054; 30 s is the alternative pending D-KN3, package decision 37), keying stops with the sidetone continuing and "PADDLE?" displayed, and transmit re-arms when both paddle contacts are confirmed open (KEY inhibit, row 3). The time limit counts elapsed time from the start of the first of the identical elements, spaces included, not key-down time: 128 dits take 6.1 s elapsed at 50 WPM (3.1 s of it keyed), 20.5 s at 15 WPM and 61 s at 5 WPM, so with the 10 s limit the element count acts first above about 31 WPM and the time limit below it (with the 30 s alternative, the crossover is about 10 WPM); with the 10 s limit a held dah lever reaches the time limit first at every speed (128 dahs take 12.3 s elapsed at 50 WPM). The same basis is raised against the REQ-SYS-054 statement (Appendix D item D18); the independent cutoff does not see this case because every element starts a new key-down, so this watchdog is the control.
...
6. *Headphones or a shorted TRS cable inserted after boot:* in Straight mode the closure is a continuous key-down and step 2 applies. In an iambic mode both paddle inputs read closed, which the keyer answers as a squeeze with an alternating dit-dah stream; every element is a new key-down, so the independent cutoff of step 4 never acts, and REQ-SYS-054 as written counts identical elements only. The proposed no-gap watchdog of HZ-004 K4 stops such a stream when both contacts stay closed for more than 2 s, or when TX_KEY has had no key-up gap of at least 7 dit times (or 500 ms) for 30 s, with the sidetone continuing and "PADDLE?" shown (package decision 37, D-KN3; REQ-SYS-054 is reworded if adopted). Independent of firmware, the transmission-length backstop of REQ-SYS-180 ends RF 150 s to 180 s after the T/R drive enters transmit if package decision 38 adopts it. If both are declined, firmware alone bounds a toggling stream (`docs/safety/hazard-analysis.md` section 8.2 row 3; OQ-SAF-006).
**Expected outcome:** no continuous key-down longer than 13 s from any single cause (the 13 s bound of the independent cutoff covers a continuous key-down only; a toggling stream is bounded by the firmware watchdog and, if adopted, by REQ-SYS-180), the specific alert on the display, recovery by itself for a key cause once the contact opens, and a latched fault only when the protection itself has been exercised; coverage per `docs/research/keyer-verification-and-key-input-network.md` F13.
...
| Hang | 8 dits default; operator-set 3 to 30 dits (REQ-SYS-044, ADR-026; the fixed 50 to 2500 ms and 6.1-dit options are withdrawn); referenced to displayed WPM in every mode | keyer-verification F11, D-KN4; SI-036 | Semi break-in fixed by SI-036; values pending |
| Stuck-key controls | Interlock 500 ms; straight-key timeout 5 s (2 to 6 s); paddle watchdog 128 consecutive identical elements or 10 s of them in elapsed time (REQ-SYS-054, TBR; 30 s is the alternative of package decision 37); independent cutoff 10 s (7.5 to 13 s); watchdog reset within 2 s | keyer-verification F13, D-KN2, D-KN3, D-KN8 | Pending (recommended adopt all, with the tune row) |
| Inhibit and latched classes | Table 3.4-4; a second watchdog reset without a switch-off latches Fault-safe | This document, section 3.4 | Pending (owner ratifies the table at SRR) |
...
| Receiver set | -6 dB 500 Hz +100/-50 Hz; -60 dB at most 2.5 kHz; 60 dB at +/-2 kHz; ultimate 80 dB; ripple 2 dB; MDS -140 dBm (target -142); no pumping from -60 dBm at +/-2 kHz; BFO offset calibration range +/-500 Hz in 10 Hz steps, stored per unit (the operator sets the pitch through the sidetone, 300 to 1000 Hz) | `docs/research/cw-selectivity-options.md` implications 1 to 4 | TBR set; PDR trade between candidates A and B (close_by PDR) |
...
| Low-battery and cell-temperature limits | Transmit cutoff per cell 3.20 V +/-0.05 V, either cell (6.4 V for a balanced pack; REQ-SYS-097); power-down per cell 3.00 V (6.0 V balanced; REQ-SYS-098); cell power-down 60 C | REQ-SYS-097 to REQ-SYS-099; power F5 | TBR (close_by PDR) |
| Battery life | At least 8 h at 1:9 and 6 h at 1:4 on the TPM-008 cycle, to the REQ-SYS-097 per-cell transmit cutoff (6.4 V for a balanced pack) | SI-034; power F23; TPM-008 | 8 h fixed by SI-034; 6 h pending |
...
This revision changes behaviour that L1 requirements, hazards, plans and research cite. The ConOps author does not edit those files; each item below is raised to the named owner, to be entered as a RID against the product at SRR (charter section 4) or fixed before it. The status column records, for each item, what the named products already carry (SRR package item R14); an item marked Edited has its text in the products and closes with the package decision named. Until an item closes, this document governs the behaviour it describes (REQ-SYS-002 makes section 3.4 binding).

| # | Product | Item | Proposed resolution | Status (2026-09-26, products at commit 400e59d) |
|---|---|---|---|---|
| D1 | REQ-SYS-002 | The mode set changed: Guest-locked is now the GUEST flag and Bench-test is a mode; the count stays nine; the forbidden transitions are F1 to F9 | Update the rationale's mode list and forbidden-transition summary; add OPS-022 to its sources | Closed: REQ-SYS-002 rationale lists the nine modes, the four flags and F1 to F9, and its sources include OPS-022 |
| D2 | REQ-SYS-003 | Key inputs are no longer a Self-test check; they are the KEY inhibit (REQ-SYS-052) | Rationale edit only | Closed: REQ-SYS-003 rationale states that a closed key input is the KEY inhibit of REQ-SYS-052, not a Self-test check |
| D3 | REQ-SYS-004 | Carriers end through the shaped fall for the Inhibit class and the flags as well as the Latched class | REQ-SYS-004 now covers "any inhibit, flag or latched fault": RF to the RF-off level of REQ-SYS-183 (TBR) within 20 ms (TBR), and this document states the same value (Table 3.4-1 Transmit-keyed row, Table 3.4-4 preamble, OPS-002 branch A). The 10 ms of the previous draft is withdrawn: at the 8 ms envelope setting -40 dBc is reached 12.7 ms into the fall (REQ-SYS-004 rationale). The TBR closes with the sequencer timing table of ICD-TX-SW at PDR | Closed: see the proposed resolution column |
| D4 | REQ-SYS-005 | Only the Latched class of Table 3.4-4 enters Fault-safe; exit is T24 through Self-test or T25 | Scope the statement to latched causes and cite T24 and T25 in the rationale | Closed: REQ-SYS-005 statement is scoped to the latched cause, and its rationale names the Latched class (Table 3.4-4 rows 10 to 15) and the exits T24 and T25 |
| D5 | REQ-SYS-053, REQ-SYS-054 | Both are Inhibit-class, self-clearing; the paddle watchdog clears when both contacts are confirmed open | REQ-SYS-054: add the clear condition; rationales cite Table 3.4-4 rows 2 and 3 instead of Fault-safe | Edited: REQ-SYS-053 and REQ-SYS-054 rationales cite Table 3.4-4 rows 2 and 3 as Inhibit class, self-clearing, and REQ-SYS-054 states the clear condition "until both contacts read open"; the values close with package decision 37 |
| D6 | REQ-SYS-130; `docs/safety/hazard-analysis.md` section 7 row l | Mute on every fault conflicts with the stuck-key alarm (the sidetone continues, rows 2 and 3) | Limit the full safe state (with mute and charge disable) to resets, panics and latched faults; the key-down timeouts apply the transmit part only (amplifier off, key up, antenna to receive) | Closed for REQ-SYS-130: the full safe state applies to resets, panics and latched faults, and Inhibit-class causes, the key-down timeouts included, apply the transmit part only. Edited for `docs/safety/hazard-analysis.md` section 7 rows c and l (the SWE-134 table, formerly cited here as section 6 item l) at revision 0.4.2-pha (2026-09-26): the full safe state applies to resets, panics, latched faults and power-down, and the key-down timeouts and other Inhibit-class causes apply the transmit part only; closes when the INSP-008 reviewer verifies the committed file |
| D7 | REQ-SYS-020; REQ-SYS-055 rationale; HZ-001 K2, HZ-006 K2; `docs/design/concept.md` sections 4, 4.1, 8, 14 item 2 | Tune 10 s +/-1 s overlaps the 7.5 to 13 s cutoff window; the REQ-SYS-055 rationale "keeps above ... the 10 s tune carrier" is false | Joint RFX-D3 and D-KN2 decision (Appendix C): under option a, REQ-SYS-020 becomes 5 s +/-0.5 s (TBR) and the REQ-SYS-055 rationale says the window stays above every firmware limit including the tune; under option b, the cutoff window and HZ-001 K3, HZ-003, HZ-004 K5 change | Edited: REQ-SYS-020 (at most 5.5 s, TBR), the REQ-SYS-055 rationale (7.5 s floor 2 s above the tune bound), HZ-001 K2, HZ-006 K2 and `docs/design/concept.md` sections 4.1, 6, 8 and 14 item 2 carry option a; closes with package decision 36 |
| D8 | REQ-SYS-007 | The rationale's pending note about an operator-requested automatic ID is superseded (section 3.5.1 item 6); "confirmed bench test mode" is now the Bench-test mode of section 3.4 | Drop the pending note; cite section 3.4 Bench-test; keep the exception for machine-generated Morse into the dummy load | Closed: REQ-SYS-007 rationale drops the pending note, cites the Bench-test mode of section 3.4 (REQ-SYS-179) and keeps the machine-generated Morse exception into the dummy load |
| D9 | REQ-SYS-065, REQ-SYS-066 rationales; ADR-015; HZ-006 | "No risk of unlicensed transmission" overstates the guest lock | Restate as "no transmission by accident; release is a deliberate licensee action governed by the operator rules card"; record the residual in HZ-006 as an accepted operator-rule dependency, or adopt option b of Appendix C | Closed for REQ-SYS-065 (restated as no transmission by accident, release a deliberate licensee action under the rules card) and REQ-SYS-066; the HZ-006 operator-rule dependency is recorded as the accepted single point of `docs/safety/hazard-analysis.md` section 8.2 row 20. Open for ADR-015 section 2, which still reads "no risk of unlicensed transmission" (ADR author, with package decisions 17 and 19) |
| D10 | REQ-SYS-052, REQ-SYS-056; HZ-004 K2 | With a mono plug the ring never opens, so "after the interlock has passed" cannot be met before Straight-on-tip is chosen | State that the key-input mode may be selected while the KEY inhibit is active and that the interlock then re-runs on the newly active inputs (OPS-013 step 1) | Closed for REQ-SYS-052 (interlock re-runs after a key-mode change), REQ-SYS-056 (selection accepted while the KEY inhibit is active, appendix D item D10) and REQ-SYS-163 (selection accepted while a key input reads closed). Edited for HZ-004 K2 in `docs/safety/hazards.json` 0.4.2-pha (2026-09-26): the selection is accepted while the KEY inhibit is active (REQ-SYS-163) and the REQ-SYS-052 interlock then re-runs on the newly used inputs; closes when the INSP-008 reviewer verifies the committed file |
| D11 | `docs/safety/hazards.json` phases | Phase names differ from the modes (Transmit, Firmware load) and Self-test, Bench-test and Fault-safe have no phase | Adopt Table 3.4-5 as the mapping, or rename the phases to the mode names | Open: `docs/safety/hazards.json` phases unchanged (hazard analysis author) |
| D12 | HZ-001 K6 | K6 names the handheld-at-face off-nominal scenario | Cite OPS-022 | Open: HZ-001 K6 does not cite OPS-022 (hazard analysis author) |
| D13 | `docs/plan/tpm.json` MOP-001, MOP-002, TPM-001, TPM-016 | They parent to MOE-001 (contact range) | Re-parent to MOE-013 on the OQ-SE-005 decision | Open: MOP-001, MOP-002, TPM-001 and TPM-016 parent to MOE-001; closes with package decision 96 (OQ-SE-005) |
| D14 | `docs/research/rf-exposure-evaluation.md` F3, reading sentence under Table 2 | "All rows for the 0.5 W and 1 W steps put the general-population distance at 0.29 m or less even for a continuous carrier" is wrong: Table 2 gives 0.41 m for a 1 W carrier | Correct the sentence to 0.41 m for a 1 W carrier (0.26 m or less while keying CW at 1 W; 0.29 m for the 0.5 W carrier) so that ADR-014, HZ-006 and the handbook do not inherit it | Open: the F3 reading sentence is unchanged (research author) |
| D15 | ADR-014 | It asserts the occupational tier "as licensees under 97.13(c)(1)", which holds under OPS-A but not by the household clause under OPS-B | Cite the OET 65 Supplement B controlled-environment statement and the OPS-B tier row of Appendix C | Open: ADR-014 section 2 unchanged; closes with package decision 18 |
| D16 | L1 source coverage (02 T-20) | OPS-022 and MOE-013 are new; T-20 wants every OPS heading cited by a requirement or TC-VAL case and every MOE to reach a MOP by PDR | Cite OPS-022 from REQ-SYS-121 (RF exposure evaluation) and MOE-013 from REQ-SYS-102, REQ-SYS-103 and REQ-SYS-114 to REQ-SYS-117 | Closed: REQ-SYS-121 sources include OPS-022; REQ-SYS-102, REQ-SYS-103 and REQ-SYS-114 to REQ-SYS-117 cite MOE-013 in their sources and rationales |
| D17 | `docs/process/04-verification-and-validation.md` section 6.3 | MOE-010's bench part needs a weak-signal source that 04 section 6 does not establish; MOE-006's span needs the tinySA Ultra's lower frequency limit confirmed | Open an OQ-VV question for the weak-signal source (close by PDR); confirm the span in the tinySA TV record | Open: no OQ-VV question for the weak-signal source yet (V&V plan author) |
| D18 | REQ-SYS-054 | The statement's "10 s (TBR) of them" does not say whether the time counts elapsed time or key-down time; the rationale's figures (128 dits in 20.5 s at 15 WPM) count elapsed time with spaces (OPS-013 step 3) | State the basis in the statement, for example "10 s (TBR) elapsed from the start of the first of them", in the same change that records D-KN3 | Edited: REQ-SYS-054 states "10 s (TBR) elapsed since the first"; the value closes with package decision 37 |
