...
| D3 | REQ-SYS-004 | Carriers end through the shaped fall for the Inhibit class and the flags as well as the Latched class | Closed: REQ-SYS-004 now covers "any inhibit, flag or latched fault": RF to the RF-off level of REQ-SYS-183 (TBR) within 20 ms (TBR), and this document states the same value (Table 3.4-1 Transmit-keyed row, Table 3.4-4 preamble, OPS-002 branch A). The 10 ms of the previous draft is withdrawn: at the 8 ms envelope setting -40 dBc is reached 12.7 ms into the fall (REQ-SYS-004 rationale). The TBR closes with the sequencer timing table of ICD-TX-SW at PDR |
